...
class ScrollBox(tkinter.Listbox):
    ...
    def grid(self, row, column, sticky='nsew', rowspan=2, columnspan=1, **kwargs):
...
class DataListBox(ScrollBox):
    ...
    def requery(self, link_value=None):
        if link_value:
            ...
            print(sql)  #TODO remove this
            self.cursor.execute(sql, (link_value,))
        else:
            print(self.sql_select + self.sql_sort)   #TODO remove this line
...
def get_album(event):
    ...
    songLV.set(("please select an album",))
...
mainWindow.columnconfigure(3, weight=1)   # spacer column pn right
...
albumList.bind('<<ListboxSelect>>', get_song)
